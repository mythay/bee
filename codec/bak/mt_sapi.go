...
type AppRegisterReqFormat struct {
	...
	DeviceVersion      uint8
	Unused             uint8
	InputCommandsNum   uint8
	InputCommandsList  [255]uint16
	OutputCommandsNum  uint8
	OutputCommandsList [255]uint16
}
...
func (config *WriteConfigurationFormat) marshall() []byte {
	buf := &bytes.Buffer{}
	binary.Write(buf, binary.LittleEndian, config.ConfigId)
	binary.Write(buf, binary.LittleEndian, config.Len)
	for i := uint8(0); i < config.Len; i++ {
		binary.Write(buf, binary.LittleEndian, config.Value[i])
	}
	return buf.Bytes()
}

func (config *WriteConfigurationFormat) unmarshall(p []byte) error {
	if len(p) > 2 {
		config.ConfigId = p[0]
		config.Len = p[1]
		copy(config.Value[:], p[2:])
		return nil
	}
